...
// Point de terminaison pour récupérer la liste de bières
router.get('/', (req, res) => {
  // Récupérer toutes les bières de la base de données
  db.query('SELECT * FROM product_number', (err, results) => {
    if (err) {
...
router.post('/', (req, res) => {
  ...
  // Vérifier si la productId existe déjà
  db.query('SELECT * FROM product_id WHERE product_id = ?', [productId], (err, results) => {
    if (err) {
      ...
      const currentDate = new Date(); // Obtenez la date actuelle au format ISO
